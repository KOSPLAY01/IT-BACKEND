  ...
  async (req, res) => {
    ...
    `;
      ...
      if (assignment.deadline && new Date() > new Date(assignment.deadline)) {
        return res
          .status(400)
          .json({ error: "Deadline has passed. Submission not allowed." });
      }
      ...
      INSERT INTO submissions (
        ...
        req.user.track
      }, ${topic}, CURRENT_DATE, CURRENT_TIME,
...
app.get("/user/submissions", authenticateToken, async (req, res) => {
  try {
    ...
    if (track)
      baseQuery = sql`${baseQuery} AND LOWER(a.track) = LOWER(${track})`;
    if (startDate && endDate)
      baseQuery = sql`${baseQuery} AND s.date BETWEEN ${startDate} AND ${endDate}`;
    else if (startDate)
      baseQuery = sql`${baseQuery} AND s.date >= ${startDate}`;
...
app.get("/admin/users", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (name)
      baseQuery = sql`${baseQuery} AND LOWER(name) LIKE LOWER(${`%${name}%`})`;
...
app.get("/admin/assignments", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (track) baseQuery = sql`${baseQuery} AND LOWER(track) = LOWER(${track})`;
    if (startDate && endDate)
      baseQuery = sql`${baseQuery} AND date BETWEEN ${startDate} AND ${endDate}`;
...
app.get("/admin/assignments/:id", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (track)
      baseQuery = sql`${baseQuery} AND LOWER(u.track) = LOWER(${track})`;
    if (startDate && endDate)
      baseQuery = sql`${baseQuery} AND s.date BETWEEN ${startDate} AND ${endDate}`;
    else if (startDate)
      baseQuery = sql`${baseQuery} AND s.date >= ${startDate}`;
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch assignment and submissions" });
  }
});
...
app.get("/admin/submissions", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (assignmentId)
      baseQuery = sql`${baseQuery} AND s.assignment_id = ${assignmentId}`;
    if (track)
      baseQuery = sql`${baseQuery} AND LOWER(u.track) = LOWER(${track})`;
    if (startDate && endDate)
      baseQuery = sql`${baseQuery} AND s.date BETWEEN ${startDate} AND ${endDate}`;
...
app.put("/admin/checkin/:id/status", authenticateToken, async (req, res) => {
  try {
    ...
    if (status === "approved") {
      const [user] =
        await sql`SELECT * FROM users WHERE id = ${updated.user_id}`;
      if (user) {
        await sql`
...
app.get("/admin/attendance/csv", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // normalize incoming date to YYYY-MM-DD so different formats (ISO, locale) match stored dates
    const dateOnly = date
...
app.get("/admin/attendance", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // normalize date to YYYY-MM-DD for robust matching
    const dateOnly = date
      ? new Date(date).toISOString().slice(0, 10)
      : new Date().toISOString().slice(0, 10);
    ...
    query = sql`${query} AND date::date = ${dateOnly}`;
